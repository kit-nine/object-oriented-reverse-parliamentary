...
MAX_ABS_DIFF = 24 # Default:  24 (Causes ~1% pass rate)
INFL = 0
# Classes
# Voter class
...
class Legislator: # The legislator class will have methods of breaking a tie in the contra domus and contrarum senatum elections, introducing a bill, vote on a bill as a member of a subcommittee, vote on a bill as a member of a committee, schedule a bill's consideration, debate on a bill, override a veto with a 2/3 vote of each chamber, and send a bill to the executive for reviewing
    ...
    def vote_on_final_bill(self, bill):
        if abs(bill.pos - self.pos) <= MAX_ABS_DIFF:
            return True
        else:
            return False
...
def congress_lawmaking(congress_members, h_o_r_members, senate_members):
    ...
    if bill.alive == True:
        ...
        for i in h_o_r_members:
            if abs(bill.pos - i.pos) > MAX_ABS_DIFF:
                house_votes.append(False)
            else:
                house_votes.append(True)
        if house_votes.count(False) > house_votes.count(True): bill.alive = False
        ...
        if senate_votes.count(False) > senate_votes.count(True): bill.alive = False
        ...
        if final_votes.count(True) <= final_votes.count(False):
            bill.alive = False
    return bill

def executive_lawmaking(executive, bill):
    if abs(bill.pos - executive.pos) > MAX_ABS_DIFF:
        executive.veto(bill)
    else:
...
while year <= RUNTIME:
    ...
    while day <= 365:
        for i in range(0, 44):
            ...
            if bill.alive == True:
                bill, vetoed = executive_lawmaking(contra_domus, bill)
            if vetoed == True:
                congress_override(bill, congress_members)
            if bill.alive == True:
                bill = judicial_review(bill, justices)
            if bill.alive == True:
...
print("Contra Domus Position:", contra_domus.pos)
print("Contrum Senatum Position:", contrum_senatum.pos)
print("Senate:", senate_pos)
print("House Position:", h_o_r_pos)