...
def senate_voting(voters):
    ...
    for state in range(1,51):
        for i in range(1,3):
            senator = Senator(Legislator(STATES.get(state), senate_positions[state*i]))
...
def h_o_r_voting(voters):
    ...
    for state in range(1,51):
        for i in range(1,REPS_PER_STATE.get(STATES.get(state - 1)) + 1):
            representative = Legislator(STATES.get(state), h_o_r_positions[state*i])
...
def c_d_voting(voters, h_o_r_positions):
    c_d_votes = []
    c_d_counts = [0]
    ...
    # tiebreaker here
    contra_domus = c_d_max_indices[0]
    return contra_domus, h_o_r_pos
# voting for the contrum senatum
def c_s_voting(voters, senate_positions):
    c_s_votes = []
    c_s_counts = [0]
    ...
    # tiebreaker here
    contrum_senatum = c_s_max_indices[0]
    return contrum_senatum, senate_pos
